...
{
    /*
    [self setCurrentViewController:self.currentPage];
    return;
    [self loadControllerAtIndex:self.currentPage andPlaceAtIndex:0];
    
    CGFloat size =(_direction==DMLazyScrollViewDirectionHorizontal) ? self.frame.size.width : self.frame.size.height;
    [self setContentOffset:[self createPoint:size*2] animated:NO];
     */
}
